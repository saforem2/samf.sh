...
So the commands live in one `start-services` script that restarts each agent
**and then probes the port a real consumer would use**:
...
The script exits with the number of failed checks, which makes it usable from
`cron` or a healthcheck as well as by hand:

```text
host: mbph

verifying
  Scrollback terminal history        ok (HTTP 200)
  SilverBullet                       ok (HTTP 200)
  ai-memory server                   ok (HTTP 401)
  VaultServe                         ok (HTTP 200)
  Tailcat listener                   ok (pid 95304)
  Herdr relay                        ok (HTTP 401)
  Cloudflare tunnel                  ok (pid 46219)
  ai-memory spool drainer            ok (periodic, idle)
  ai-memory -> SilverBullet export   ok (periodic, idle)

all checks passed
```

The AGPT dashboard and the Aurora/CELS forwards are still started when I need
them. The LaunchAgent-managed services return after login without an open
...
> through it hung instead of failing. Run `start-services --check` on *both*
